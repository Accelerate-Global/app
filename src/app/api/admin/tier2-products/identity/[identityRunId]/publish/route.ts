...
export const POST = withRoute(
  { access: "admin", action: "publish Tier 2 identity candidates" },
  async (identity, request: Request, context: Context) => {
    ...
    try {
      return Response.json(await publishTier2PartnerIdentityCandidate({
      ...
      return tier2ProductRouteError(
        "Failed to publish Tier 2 identity candidate",
        "Could not publish the Tier 2 identity candidate.",
        error,
      );
    }
  },
