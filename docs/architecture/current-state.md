...
- Every `src/app/api/**/route.ts` handler is wrapped in `withRoute` from `src/lib/route-guard.ts`, which owns identity resolution, 401/403 gating, and last-resort error normalization; `src/lib/route-guard-sweep.test.ts` enforces adoption. Documented exemption: the analytics events route for anonymous events.
