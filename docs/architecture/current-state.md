...
- The admin-only `/api/chat` route accepts bounded user/assistant history,
  obtains a schema-constrained semantic plan from private Qwen, validates and
  deterministically compiles approved catalog keys, executes through the
  read-only analytics broker, and streams a grounded answer with provenance.
