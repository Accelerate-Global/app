...
- The safe analytics projection and catalog gain normalized ROP3 identity plus reviewed ROP hierarchy/classification fields. A server-owned relationship registry resolves the immutable ROP version from dataset production lineage or, only for a reviewed pre-publication dataset version, an explicit append-only legacy binding; it compiles a null-preserving many-to-one relationship and fails closed when neither binding can be proven.
