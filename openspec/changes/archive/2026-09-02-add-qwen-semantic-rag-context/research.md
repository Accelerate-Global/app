...
On August 31, 2026, Blake approved the 30-minute view-context lifetime and all proposed retrieval quality/resource gates. Blake is the semantic conflict approver for the pilot and directed that runtime definition resources and their human guiding documents remain synchronized through one bidirectional candidate workflow. Blake subsequently approved complete governed ROP browsing, reviewed ROP filtering/grouping, and registered ROP relationships as phase-one scope. “Unrestricted” is recorded as complete user-facing access within the authenticated ROP and primary-dataset scope; it does not authorize model-written SQL, physical join conditions, lifecycle mutation, or unregistered relationships. Standalone browsing uses the labeled active ROP version, whereas dataset relationships must use the immutable ROP version bound through production lineage and fail closed without it. Blake also approved the null-preserving rule for both the interactive UUPG view and unfiltered people-group-to-ROP3 relationship results. The UUPG quick reference will visibly explain its null-preserving behavior and distinguish the interactive filter from the stricter Baseline UUPG pipeline.
...
- Added a typed ROP resource-query branch, complete page-by-page catalog access, reviewed ROP analytics fields, immutable dataset-version binding, a server-owned null-preserving ROP3 relationship, grain-safe geography operations, and separate ROP entry-retrieval/cardinality/security evaluation.
