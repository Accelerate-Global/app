...
The system SHALL use `admin`, `pro`, and `basic` as the canonical workspace roles. Legacy stored role value `viewer` MUST resolve as `pro` for compatibility, but new role writes MUST use `pro`.
...
The system SHALL prevent `basic` users from changing account profile name, email, or disabling their own account through both UI-visible controls and server/database enforcement.
...
The system SHALL preserve current standard-user account capabilities for `pro` users.
