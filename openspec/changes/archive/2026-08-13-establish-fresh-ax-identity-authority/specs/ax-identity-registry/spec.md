...
**Reason**: AX Online is intentionally starting with a new authority and must not import or consult AX Data identity material.
**Migration**: Remove the importer, manifests, tables, functions, triggers, artifacts, and commands after a guarded zero-state production preflight.

### Requirement: Authoritative identity work requires cutover evidence
**Reason**: A neutral empty-authority marker replaces the legacy graph cutover marker.
**Migration**: Use the state-bound CLI initialization defined by `ax-identity-authority-initialization`.
