...
The system SHALL support structured `clarify`, `query`, and non-data `answer` decisions and MUST NOT offer database writes, pipeline actions, exports, account actions, or arbitrary tools.

#### Scenario: Question is ambiguous
- **WHEN** the supported meaning, dataset, metric, dimension, or time scope cannot be resolved deterministically
...
The system SHALL ground data answers only in the bounded broker result and SHALL report the approved dataset/catalog version, applied filters, row count, and query identifier. It MUST distinguish empty results from unavailability and MUST NOT invent causes or unseen facts.

#### Scenario: Query returns data
- **WHEN** an admitted query returns bounded rows
- **THEN** the assistant explains only facts supported by those rows and the response includes provenance
